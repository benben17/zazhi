...
class BaseHandler(object):
        ...
        for i in range(SENDMAIL_RETRY_CNT+1):
            try:
                ...
                if i < SENDMAIL_RETRY_CNT:
                    ...
                    default_log.warn(
                        'overquota when sendmail to %s:%s' % (to, str(e)))
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='over quota')
                    break
                ...
                self.deliverlog(name, str(to), title, len(
                    attachment), tz=tz, status='wrong SRC_EMAIL')
                break
                ...
                if SENDMAIL_ALL_POSTFIX:
                    ...
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='invalid postfix')
                    break
                ...
                if i < SENDMAIL_RETRY_CNT and sgenable and sgapikey:
                    ...
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='sendgrid timeout')
                    time.sleep(5)
                else:
                    default_log.warn(
                        'timeout when sendmail to %s:%s, abort!' % (to, str(e)))
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='timeout')
                    break
            except Exception as e:
                if i < SENDMAIL_RETRY_CNT and sgenable and sgapikey:
                    default_log.warn(
                        'sendgrid sendmail to %s failed:%s. Try google api.' % (to, str(e)))
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='sendgrid failed')
                else:
                    default_log.warn(
                        'sendmail to %s failed:%s. ' % (to, str(e)))
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='send failed')
                    break
            else:
                if i < SENDMAIL_RETRY_CNT and sgenable and sgapikey:
                    self.deliverlog(name, str(to), title, len(
                        attachment), tz=tz, status='sendgrid ok')
                else:
                    self.deliverlog(name, str(to), title,
                                    len(attachment), tz=tz)
                break
            ...
            try:
                if attachments:
                    if html:
                        ...
                        mail.send_mail(SRC_EMAIL, to, title,
                ...
                default_log.warn(
                    'overquota when sendmail to %s:%s' % (to, str(e)))
                self.deliverlog(name, str(to), title, 0,
                                tz=tz, status='over quota')
                break
            except InvalidSenderError as e:
                default_log.warn(
                    'UNAUTHORIZED_SENDER when sendmail to %s:%s' % (to, str(e)))
                self.deliverlog(name, str(to), title, 0, tz=tz,
                ...
                if i < SENDMAIL_RETRY_CNT:
                    ...
                    default_log.warn(
                        'timeout when sendmail to %s:%s, abort!' % (to, str(e)))
                    self.deliverlog(name, str(to), title, 0,
                                    tz=tz, status='timeout')
                    break
            except Exception as e:
                default_log.warn('sendmail to %s failed:%s.<%s>' %
                                 (to, str(e), type(e)))
                self.deliverlog(name, str(to), title, 0,
                                tz=tz, status='send failed')
                break
            else:
                if attachments:
                    size = len(html or textcontent) + \
                        sum([len(c) for f, c in attachments])
                else:
                    size = len(html or textcontent)
    ...
    def render(self, templatefile, title='KindleEar', **kwargs):
        ...
        html = main.jjenv.get_template(
            templatefile).render(title=title, **kwargs)
