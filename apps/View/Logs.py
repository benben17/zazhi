...
class Mylogs(BaseHandler):
    ...
    def GET(self):
        ...
        try:
            mylogs = DeliverLog.all().filter(
                "username = ", user.name).order('-time').fetch(limit=10)
        ...
        if user.name == 'admin':
            for u in KeUser.all().filter("name != ", 'admin'):
                try:
                    ...
                    ul = sorted(DeliverLog.all().filter(
                        "username = ", user.name), key=attrgetter('time'), reverse=True)[:5]
                if ul:
                    logs[u.name] = ul
        ...
        if user.name == 'admin':
            try:
                ...
                lastDelivered = sorted(LastDelivered.all().fetch(
                ), key=attrgetter('datetime'), reverse=True)[:100]
        else:
            try:
                lastDelivered = LastDelivered.all().filter(
                    'username = ', user.name).order('-datetime').fetch(limit=100)
            except NeedIndexError:
                lastDelivered = sorted(LastDelivered.all().filter(
                    'username = ', user.name), key=attrgetter('datetime'), reverse=True)[:100]
...
class RemoveLogs(BaseHandler):
    ...
    def GET(self):
        ...
        query.filter('datetime < ', datetime.datetime.utcnow() -
