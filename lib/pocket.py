...
class Pocket(object):
    ...
    def _post(self, method_url, **kw):
        ...
        if ret.status_code != 200 or not ret.content:
            raise APIError(ret.status_code, ret.headers.get(
                'X-Error-Code', ''), ret.headers.get('X-Error', ''), 'Get access token')
    ...
    def get_request_token(self):
        ...
        ret = self._post(
            REQUEST_TOKEN_URL, consumer_key=self.consumer_key, redirect_uri=self.redirect_uri)
        return ret.get('code', '')

    def get_authorize_url(self, code):
        if not self.redirect_uri:
            raise APIError(400, '140', 'Missing redirect url.',
                           'Get access token')
        url = AUTH_TOKEN_URL % {'request_token': code,
                                'redirect_uri': self.redirect_uri}
        return url

    def get_access_token(self, code):
        # access token : {"access_token":"dcba4321-dcba-4321-dcba-4321dc","username":"pocketuser"}.
        ret = self._post(ACCESS_TOKEN_URL,
                         consumer_key=self.consumer_key, code=code)
